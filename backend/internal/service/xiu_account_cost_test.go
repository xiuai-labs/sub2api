...
func (r *xiuCostStoreStub) GetXiuAccountTotalCostBatch(_ context.Context, accountIDs []int64) (map[int64]*usagestats.AccountStats, error) {
	r.gotIDs = accountIDs
	return r.rows, r.err
}

func TestGetXiuTotalCostBatch_FillsZeroForAccountsWithoutRows(t *testing.T) {
	repo := &xiuCostStoreStub{rows: map[int64]*usagestats.AccountStats{
...
func TestGetXiuTotalCostBatch_PropagatesQueryError(t *testing.T) {
	repo := &xiuCostStoreStub{err: errors.New("statement timeout")}
	svc := &AccountUsageService{usageLogRepo: repo}
...
func TestGetXiuTotalCostBatch_RejectsRepoWithoutStore(t *testing.T) {
